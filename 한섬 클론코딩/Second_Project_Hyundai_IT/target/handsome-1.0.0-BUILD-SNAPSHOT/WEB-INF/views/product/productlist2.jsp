				...
				.ready(
						function() {
							...
							categoryListList = new BoardListController(
									'categoryListForm', '#listBody', '.paging',
									12); //폼아이디, 리스트공간 셀렉터, 페이징공간 셀렉터, 페이지당 리스트개수
							...
							function deactive(item) {
								var $item_box = $(item);
								$item_box.find('.item_size').stop().slideUp(
										'fast');
...
<div class="paging" style="display: block;">
	<c:if test="${pageMaker.prev}">
		<!-- 이전 버튼 -->
			<a href="/list?clarge=${category.clarge}&cmedium=${category.cmedium}&csmall=${category.csmall}&pageNum=${pageMaker.startPage - 1}" class="prev">Previous</a>
	</c:if>
...
<div id="criteoVariable">
	<script type="text/javascript">
		...
		for(i=0; i< products.length ; i++){
		    const img1 = products[i].getElementsByClassName("cl")[0].getAttribute("cimage1");
		    const img2 = products[i].getElementsByClassName("cl")[0].getAttribute("cimage2");
