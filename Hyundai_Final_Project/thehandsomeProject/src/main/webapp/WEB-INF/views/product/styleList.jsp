...
<div id="bodyWrap" class="products">
	...
	<h3 class="cnts_title ou1804" >
		<span style="padding-bottom: 20px; font-weight: bold;">
			"${style}" 스타일
		</span>
		<p style="padding-bottom: 20px; font-size: 20px; letter-spacing: -1.5px">
			<c:set var = "style"  value = "${style}" />
			<c:choose>
				<c:when test = "${style eq '캐주얼'}">
					평상시에 가볍게 입을 수 있는 스타일
				</c:when>
				<c:when test = "${style eq '페미닌'}">
					여성스러우면서 우아한 분위기가 있는 스타일
				</c:when>
				<c:when test = "${style eq '모던'}">
					현대적이고 세련된 느낌을 주는 스타일
				</c:when>
				<c:when test = "${style eq '로멘틱'}">
					온순하고 부드러운 여성적인 스타일
				</c:when>
				<c:when test = "${style eq '클래식'}">
					유행을 타지않고 전통적이고 전형화된 스타일
				</c:when>
				<c:when test = "${style eq '내추럴'}">
					자연스럽게 어우러지게 하는 스타일
				</c:when>
			</c:choose>
		</p>
	...
	<!-- 카테고리 표시 끝 -->
	<div class="adaptive_wrap" style="margin-top: 20px;">
		<form id="categoryListForm" action="http://www.thehandsome.com/ko/c/categoryList">
			...
			<!-- sort wrap -->
			<div class="filterWrap hsDelivery1902" style="display: none">
				<ul class="clearfix float_left">
					...
					<li class="price" id="price_li_btn"><a
						...
						<div class="list" id="price_list_li" style="display: none;">
							<ul>
								<li><input type="checkbox" class="price_ck" id="price_ck" value="1" onclick="setPriceOrderCode(this, '1')">
									<label for="price_ck1" id="lable_ck1"> ￦100,000 이하</label></li>
								<li><input type="checkbox" class="price_ck" id="price_ck" value="2" onclick="setPriceOrderCode(this, '2')">
									<label for="price_ck2" id="lable_ck2">￦100,000 ~
										￦300,000</label></li>
								<li><input type="checkbox" class="price_ck" id="price_ck" value="3" onclick="setPriceOrderCode(this, '3')">
									<label for="price_ck3" id="lable_ck3">￦300,000 ~
										￦500,000</label></li>
								<li><input type="checkbox" class="price_ck" id="price_ck" value="4" onclick="setPriceOrderCode(this, '4')">
									<label for="price_ck4" id="lable_ck4">￦500,000 ~
										￦1,000,000</label></li>
								<li><input type="checkbox" class="price_ck" id="price_ck" value="5" onclick="setPriceOrderCode(this, '5')">
					...
					<li>
						<div class="item_box">
							<a href="" class="item_info1"
								...
								<span class="price"><span id="price_TM2CAWOT761W_BK"><span>₩<fmt:formatNumber value="${product.pprice}" pattern="#,###"></fmt:formatNumber></span></span></span>
			    ...
			    $(document).ready(
			    	    function() {
			    	        ...
			    	        $("#filterapp").click(function(){
								...
								$(location).attr('href', '/product/styleList/1_'+"${productCount}_"+filter_values+"&${style}");
			    	        });
			    	        ...
			    	        $(".pageBtn").on("click", function(e) {
			    	            ...
			    	            location.href="/product/styleList/"+$(this).attr("pagenum")+ "_${productCount}" + "${page_info_ex}" + "&${style}";
			    	        });//end click
			    	        ...
			    	        $(".prev").on("click",function(e){
			    	            ...
			    	            location.href="/product/styleList/"+(parseInt(curPageNum.attr("pagenum"))-1)+"_${productCount}" +"${page_info_ex}"  + "&${style}";
			    	        ...
			    	        $(".next2").on("click",function(e){
			    	            ...
			    	            e.preventDefault(); //<a> 작동 중지
			    	            console.log('click');
			    	            location.href="/product/styleList/${pageMaker.realEnd}"+"_${productCount}" +"${page_info_ex}" + "&${style}";
			    	        ...
			    	        $(".prev2").on("click",function(e){
			    	            ...
			    	            e.preventDefault(); //<a> 작동 중지
			    	            console.log('click');
			    	            location.href="/product/styleList/1"+"_${productCount}" +"${page_info_ex}" + "&${style}";
