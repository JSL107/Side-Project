		...
		<form id="categoryListForm" action="http://www.thehandsome.com/ko/c/categoryList">
			...
			<div class="filterWrap hsDelivery1902">
				<ul class="clearfix float_left">
					...
					<c:if test="${!ctg.startsWith('be')}">
					<li class="size" id="size_li_btn">
						<a href="javascript:void(0);" class="select">사이즈<span
							class="ico_arr">arrow</span></a>
					...
					<li class="price" id="price_li_btn">
					<a href="#" class="select">가격<span class="ico_arr">arrow</span></a>
					...
					<li class="sortby" id="sortby_li_btn"><a
						href="javascript:void(0);" class="select">정렬순<span
							class="current" id="chang_filter_categoriy">신상품</span><span
				...
				<c:if test="${ctg.startsWith('be')}">
					<li class="btn" style="margin-left: 480px;"><a id="resetapp" href="javascript:void(0);">초기화</a><a
						id="filterapp" href="javascript:reset();">적용</a></li>
				</c:if>
				...
				<c:forEach items="${productList}" var="product">
					<li>
						<div class="item_box">
							...
							<c:if test = "${ctg.startsWith('be')}">
								<c:forEach items="${product.colorList}" var="color">
									<c:if test="${not empty color.c_thumbnail2}">
										<a href="javascript:void(0);" class="cl wt"
											productid="${product.pid}" colorcode="${color.ccolorcode}"
											thumbnailimage1="${color.c_thumbnail1}"
											thumbnailimage2="${color.c_thumbnail2}"></a>
									</c:if>
									
									<c:if test="${empty color.c_thumbnail2}">
										<a href="javascript:void(0);" class="cl wt"
											productid="${product.pid}" colorcode="${color.ccolorcode}"
											thumbnailimage1="${color.c_thumbnail1}"
											thumbnailimage2="${color.c_thumbnail1}"></a>
									</c:if>
								</c:forEach>
							</c:if>
	    ...
	    $(document).ready(
	    	    function() {
	    	        ...
	    	        $("#filterapp").click(function(){
	    	            ...
	    	            //price_filter = $("input:checkbox[class='price_ck']:checked").attr('value');
